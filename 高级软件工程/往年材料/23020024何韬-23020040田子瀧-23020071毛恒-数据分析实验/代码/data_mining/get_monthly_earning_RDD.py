...
def query(row):
    ...
    for index, interval in enumerate(boundaries2):
        ...
        df = df._append({'slug':slug, 'month_index':index-9, 'earning':earning},
            ignore_index=True)
    return df
...
if __name__ == "__main__":
    ...
    for row in tqdm(dicts[:]):
        result = query(row)
        if not result.empty:
            df_money = df_money._append(result, ignore_index=True)
    df_money.to_csv('news/20190821_get_dates.csv', index=False)
