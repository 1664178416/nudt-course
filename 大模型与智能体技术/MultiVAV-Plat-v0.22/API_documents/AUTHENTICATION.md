...
### 🔓 Default Behavior
**When no API key is provided, the system defaults to the USER role.** 
However, for lowest privilege programmatic access (e.g., simple pilot agents), using the **AGENT** role is recommended.
...
curl -H "X-API-Key: system_secret_key_change_in_production" http://localhost:8000/drones
```

### 🔑 Default Secret Keys (Change in Production)
| Role | Environment Variable | Default Value |
|:---|:---|:---|
| **AGENT** | `AGENT_SECRET_KEY` | `agent_secret_key_change_in_production` |
| **USER** | `USER_SECRET_KEY` | `user_secret_key_change_in_production` |
| **SYSTEM** | `SYSTEM_SECRET_KEY` | `system_secret_key_change_in_production` |
| **ADMIN** | `ADMIN_SECRET_KEY` | `admin_secret_key_change_in_production` |
...
| `GET` | `/targets/type/{type}` | Get targets by type | **AGENT+** |
| `GET` | `/targets/waypoints` | Get charging waypoints | **AGENT+** |
| `GET` | `/targets/waypoints/nearest` | Find nearest waypoint | **AGENT+** |
| `POST` | `/targets/waypoints/{id}/check-drone` | Check charging status | **AGENT+** |
...
| `GET` | `/obstacles/type/{type}` | Get obstacles by type | **AGENT+** |
...
| Method | Endpoint | Description | Minimum Role |
|:---|:---|:---|:---:|
| `GET` | `/sessions` | Get all sessions | **SYSTEM+** |
...
| `POST` | `/sessions/{id}/set-current` | Set active session | **SYSTEM+** |
...
| `GET` | `/sessions/{id}/tasks` | Get all tasks in session | **AGENT+** |
| `POST` | `/sessions/{id}/tasks` | Create new task | **SYSTEM+** |
| `GET` | `/sessions/{id}/tasks/{id}` | Get specific task | **SYSTEM+** |
| `PUT` | `/sessions/{id}/tasks/{id}` | Update task | **SYSTEM+** |
| `DELETE` | `/sessions/{id}/tasks/{id}` | Delete task | **SYSTEM+** |
| `POST` | `/sessions/{id}/tasks/{id}/mark-done` | Mark task completed | **AGENT+** |
| `POST` | `/sessions/{id}/tasks/{id}/mark-pending` | Mark task pending | **AGENT+** |
...
| `GET` | `/check/two_drones_distance` | Check distance between drones | **ADMIN** |
...
| `GET` | `/check/target_in_photo_taken_by_drone` | Check target in photo | **AGENT+** |
