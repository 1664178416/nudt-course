...
|------|--------|-----------------|
| **USER** | Basic (control & view) | No (default) |
...
| POST | `/sessions/{id}/set-current` | Set as active |
...
| GET | `/sessions/{id}/target-reaches` | Target reach log and stats (supports multiple visits per target) |
...
**Note on Target Reaches**: Multiple visits to the same target are recorded. Each time a drone enters a target's `task_radius`, a new timestamp is added to the visit log. The data structure is: `{drone_id: {target_id: [timestamp1, timestamp2, ...]}}`.
...
| POST | `/sessions/{session_id}/tasks` | Create a new task (SYSTEM/ADMIN) |
| GET | `/sessions/{session_id}/tasks/{task_id}` | Get specific task (USER+) |
| PUT | `/sessions/{session_id}/tasks/{task_id}` | Update task (SYSTEM/ADMIN) |
| DELETE | `/sessions/{session_id}/tasks/{task_id}` | Delete task (SYSTEM/ADMIN) |
| POST | `/sessions/{session_id}/tasks/{task_id}/mark-done` | Mark task as completed (USER+) |
| POST | `/sessions/{session_id}/tasks/{task_id}/mark-pending` | Mark task as pending (USER+) |
| POST | `/sessions/{session_id}/tasks/swap` | Swap the order of two tasks (SYSTEM/ADMIN) |
| GET | `/sessions/current/tasks` | Get all tasks in current session (USER+) |
| GET | `/sessions/current/tasks/{task_id}` | Get specific task from current session (USER+) |
...
```json
{
  "command": "string (connect, disconnect, take_off, land, move_to, change_altitude, hover, rotate, return_home, set_home, calibrate, take_photo, send_message, broadcast)",
  "parameters": {}
}
```
...
{
  "name": "string",
  "target_type": "string (fixed, moving, waypoint, circle, polygon)",
  "position": {"x": "float", "y": "float", "z": "float"},
  "description": "string (optional)",
  "velocity": {"x": "float", "y": "float", "z": "float"} (optional, PRIORITY 1: if non-zero, uses velocity mode),
  "radius": "float (optional, default: 1.0)",
  "moving_path": [{"x": "float", "y": "float", "z": "float"}, ...] (optional, PRIORITY 2: used only if velocity is zero/null),
...
```json
{
  "drone_at_waypoint": "boolean",
  "waypoint_id": "string",
  "drone_id": "string",
  "distance": "float",
  "waypoint_radius": "float",
  "charge_amount": "float (optional, for charging stations)",
...
```json
{
  "waypoint": "TargetResponse or null",
  "distance": "float"
}
```
...
```json
{
  ...
  "time_mode": "string (optional, default: time_agnostic) - 'time_related' or 'time_agnostic'",
...
```json
{
  ...
  "target_reaches": {
    "drone1": {
      "target1": [1704067300.0, 1704067400.0],
      "target2": [1704067350.0]
    }
  },
  "target_reach_statistics": {
    "total_reaches": 5,
    ...
    "reaches_by_drone": {
      "drone1": [
        {"target_id": "target1", "target_name": "Zone A", "distance": 5.2, "timestamp": 1704067300.0}
      ]
    },
    "reaches_by_target": {}
  },
...
- Response: Binary content with media type `image/png`, `image/jpeg`, or `application/pdf` based on `format`.
...
### Target Types
- `fixed` - Static waypoint (can also be used for points of interest)
...
Query: `drone_id_1`, `drone_id_2`, `max_distance?` (optional), `min_distance?` (default 0)
Response: `result` (bool), `value` (distance), `drone_id_1`, `drone_id_2`.
...
Query: `drone_id`, `min_altitude?` (default 5.0), `since_timestamp?`
Response: `result` (has taken off), `value` (number of takeoff events), `takeoff_events` (list of events with positions and timestamps).

**GET** `/check/drone_has_landed` — check if drone has landed in history
Query: `drone_id`, `max_altitude?` (default 1.0), `since_timestamp?`
Response: `result` (has landed), `value` (number of landing events), `landing_events` (list of events).
...
Response: `result` (all taken off), `value` (percentage 0-100), `taken_off_drones` (list), `not_taken_off_drones` (list), `total_drones` (count).

**GET** `/check/all_drones_have_landed` — check if all drones have landed
Query: `max_altitude?` (default 1.0), `since_timestamp?`, `check_history?` (default true)
Response: `result` (all landed), `value` (percentage 0-100), `landed_drones` (list), `not_landed_drones` (list), `total_drones` (count).
...
**GET** `/check/target_is_reached_by_drone` — specific drone reached target  
Query: `target_id`, `drone_id`  
Response: `result`, `value` (visits count), `visit_log_entries`.

**GET** `/check/target_reached_drone_number` — reached drone count vs expectation  
Query: `target_id`, `expected_count?`  
...
curl -X GET "http://localhost:8000/check/two_drones_distance?drone_id_1=drone-1&drone_id_2=drone-2&max_distance=100.0" \
  -H "X-API-Key: admin_secret_key_change_in_production"
...
# Check if all drones have landed (check history)
curl -X GET "http://localhost:8000/check/all_drones_have_landed?check_history=true&max_altitude=1.0" \
  -H "X-API-Key: admin_secret_key_change_in_production"
