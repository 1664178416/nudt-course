...
|------|-------------|------------------------|-------------|
| **USER** (Default) | Basic Access | No (defaults if no key provided) | Can control drones and view resources |
| **AGENT** | Basic Access | Yes - provide AGENT API key | Same privileges as USER |
...
To use AGENT, SYSTEM or ADMIN roles, include the `X-API-Key` header:

```bash
# Example with AGENT key
curl -H "X-API-Key: agent_secret_key_change_in_production" http://localhost:8000/drones

# Example with SYSTEM key
curl -H "X-API-Key: system_secret_key_change_in_production" http://localhost:8000/sessions
```

**Default Keys (Change in Production!):**
- USER: No key needed (or `user_secret_key_change_in_production`)
- AGENT: `agent_secret_key_change_in_production`
- SYSTEM: `system_secret_key_change_in_production`
- ADMIN: `admin_secret_key_change_in_production`
...
**Description:** Retrieves a list of all sessions in the system.
...
| time_mode | string | No | Time mode: 'time_related' or 'time_agnostic' (default: 'time_agnostic') |
...
| time_mode | string | No | Time mode (default: 'time_agnostic') |
...
**Response Fields (when data=false, default):**
- All standard session fields (id, name, description, status, time_mode, task, etc.)
...
- `history`: Object containing:
  ...
  - `target_reaches`: Drone-to-target visit tracking
...
Provides rendered screenshots of the current session UI in PNG, JPG, or PDF formats. Useful for reporting, quick previews, and exporting visual state.
...
- Query Parameters:
  - `format` (string, optional): One of `png`, `jpg`, `jpeg`, `pdf` (default: `png`).
...
- Response: Binary image/PDF content. Media type is `image/png`, `image/jpeg`, or `application/pdf` depending on `format`.
...
- Description: Returns detailed target reach log and aggregated statistics. **Multiple visits** to the same target are recorded - each time a drone enters a target's `task_radius`, a new timestamp is added to the visit log.
- Response:
  - `target_reach_log`: Array of reach records `{drone_id, drone_name, target_id, target_name, distance, timestamp}`.
  - `statistics`: Aggregates including `total_reaches`, `unique_drones`, `unique_targets`, `reaches_by_drone`, `reaches_by_target`.
- **Note**: The same drone visiting the same target multiple times will generate multiple entries with different timestamps.
...
```json
{
  "target_reaches": {
    "drone-001": {
      "target-abc": [1705449650.0, 1705449750.0, 1705449850.0],
      "target-def": [1705449800.0]
    },
    "drone-002": {
      "target-abc": [1705449700.0]
    }
  },
  "statistics": {
    "total_reaches": 4,
...
- Description: Returns task completion progress based on the session's task type. Progress calculation varies by task type:
  ...
  - **target_tracking**: Percentage of targets currently within task_radius of any drone (completed when all targets have been tracked at least once)
...
**Authentication:** Requires USER role (AGENT has same privileges, SYSTEM and ADMIN inherit)
...
SYSTEM_KEY = "system_secret_key_change_in_production"
...
**Description:** Commands a drone to move along a specified path of waypoints.
...
|------|------|----------|-------------|
| drone_id | string | Yes | ID of the drone (in URL path) |
| path | array | Yes | Array of waypoint coordinates [{x, y, z}, ...] |
...
| target_type | string | Yes | Type of target (fixed, moving, waypoint, circle, polygon) - Note: fixed type can also represent points of interest |
...
| moving_path | array | No | Array of waypoint coordinates for moving targets [{x, y, z}, ...]. **PRIORITY 2**: Used only when velocity is zero/null. Target ping-pongs along path. |
...
**Endpoint:** `GET /targets/type/{target_type}`
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| target_type | string | Yes | Type of targets to retrieve (in URL path) |
...
**Endpoint:** `GET /targets/waypoints`

**Description:** Retrieves all waypoint targets (charging stations).

**Response:** Array of waypoint objects

**Example Request:**
```bash
curl -X GET "http://localhost:8000/targets/waypoints"
```
...
**Endpoint:** `GET /targets/waypoints/nearest`

**Description:** Finds the nearest waypoint to a drone's position.
...
**Example Request:**
```bash
curl -X GET "http://localhost:8000/targets/waypoints/nearest" \
  -H "Content-Type: application/json" \
  -d '{"x": 25.0, "y": 35.0, "z": 10.0}'
```
...
| moving_path | array | No | New path waypoints for moving targets [{x, y, z}, ...]. Speed recalculated if in path mode |
...
**Note:** Changing `velocity`, `moving_path`, or `moving_duration` may switch between movement modes based on priority rules.
...
| type | string | Yes | Type of obstacle (point, circle, elliptical, polygon) |
| position | object | Yes | Position coordinates {x, y, z} |
...
**Endpoint:** `GET /obstacles/type/{obstacle_type}`
...
| Name | Type | Required | Description |
|------|------|----------|-------------|
| obstacle_type | string | Yes | Type of obstacle (point, circle, ellipse, polygon) (in URL path) |
...
- **GET** `/check/two_drones_distance` — are two drones within distance range
  - Params: `drone_id_1`, `drone_id_2`, `max_distance?`, `min_distance?`
...
- **GET** `/check/drone_has_taken_off` — check if drone has taken off in history
  - Params: `drone_id`, `min_altitude?`, `since_timestamp?`
  - `value`: number of takeoff events found
- **GET** `/check/drone_has_landed` — check if drone has landed in history
  - Params: `drone_id`, `max_altitude?`, `since_timestamp?`
...
- **GET** `/check/all_drones_have_taken_off` — check if all drones have taken off
  ...
  - `value`: percentage of drones that have taken off (0-100)
- **GET** `/check/all_drones_have_landed` — check if all drones have landed
  - Params: `max_altitude?`, `since_timestamp?`, `check_history?`
  - `value`: percentage of drones that have landed (0-100)
...
curl -X GET "http://localhost:8000/check/drone_position?drone_id=drone-1&x=50.0&y=30.0&z=5.0&tolerance=2.0" \
  -H "X-API-Key: admin_secret_key_change_in_production"
