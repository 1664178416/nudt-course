...
class ItemManager {
    ...
    async generateLevelItems(level) {
        ...
        console.log(`生成了 ${this.items.length} 个物品`); // 调试用
        console.log(`物品类型: ${this.items.map(item => item.type)}`); // 调试用
    }
    ...
    getValidPosition() {
        ...
        for (let i = 0; i < maxAttempts; i++) {
            ...
            for (const item of this.items) {
                ...
                if (Utils.checkRectCollision(newRect, itemRect)) {
                    valid = false;
                    break;
                }
            }
    ...
    checkHookCollision(hookRect) {
        for (const item of this.items) {
            if (!item.caught && Utils.checkRectCollision(hookRect, item.getRect())) {
                console.log(`捕获物品: ${item.type}, 价值: ${item.value}`); // 调试用
                item.caught = true;
                return item;
            }
        }
